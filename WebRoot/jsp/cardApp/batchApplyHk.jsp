...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#dg4").datagrid({
            url:"cardapply/cardApplyAction!batchHFSnapBatch.action",
            pagination:true,
            rownumbers:true,
            border:false,
            striped:true,
			...
			columns:[[
			    	...
			    	{field:"SURE_FLAG",title:"备注",sortable:false, formatter:function(value){
			    		if(value == '1') {
			    			...
			    			return "<span style='color:red'>人员不属于该单位</span>";
			    		} else if(value == '6') {
			    			return "<span style='color:red'>人员不存在</span>";
			    		} else if(value == '7') {
			    			return "<span style='color:red'>人员已有金融市民卡</span>";
			    		} else if(value == '8') {
			    			return "<span style='color:red'>人员没有全功能卡</span>";
			    		}
		 		...
		 		if(node.VALUE == "2"){
		 			...
		 			$grid.datagrid({
		 				...
		 				columns:[[
		 			    	...
		 			    	{field:"CORPT_YPE",title:"单位类型",sortable:true,width:parseInt($(this).width()*0.1)},
		 			    	{field:"TOTNUMS",title:"符合申领人员总数",sortable:false,width:parseInt($(this).width()*0.1)}
		 			    ]]
		 			});
		 			...
		 			$grid.datagrid({
		 				...
		 				columns:[[
		 			    	...
		 			    	{field:"CORPT_YPE",title:"",sortable:true,width:parseInt($(this).width()*0.1)},
		 			    	{field:"TOTNUMS",title:"符合申领人员总数",sortable:false,width:parseInt($(this).width()*0.1)}
		 			    ]]
		 			});
		 		}
    ...
    function viewRowsOpenDlg(){
		...
		$.modalDialog({
			...
			closable:false,
			maximizable:false,
			href:"/jsp/cardApp/batchApplyDetailView.jsp?selectedId=" + escape(encodeURIComponent(rows[0].SELECTID)),
			tools:[{
	...
	function toApply(){
		...
		var tempTitle = "您确定要将所勾选的记录进行规模申请换卡吗？<span style='color:red;'>制卡方式：" + $("#makeCardWay").combobox("getText");
		...
		$.messager.confirm("系统消息",tempTitle,function(is){
			if(is){
				...
				$.post("cardapply/cardApplyAction!saveBatchApply.action",$("#searchConts").serialize() + "&selectedId=" + selectIds,function(data,status){
					...
					if(status == "success"){
						if(data.status == "0"){
						    $.messager.alert("系统消息","申领成功！","info",function(){
						...
						$.messager.alert("系统消息","规模申请换卡出现错误，请重试！","error");
					}
				},"json");
			}
		});
	...
	function importCorp() {
		...
		$.ajaxFileUpload({  
            url:"cardapply/cardApplyAction!batchApplyImportSearch.action?rows=1000",
            fileElementId:['file'],
            data:getformdata("searchConts"),
            dataType:"json",
		...
		$.ajaxFileUpload({  
            ...
            success: function(data, status){
            	...
            	if(data.msgList && data.msgList.length > 0){
            		for(var i in data.msgList){
	            		msg += "<br>" + data.msgList[i];
            		}
            		jAlert("导入单位人员申领成功：" + msg, "info");
            	} else {
	...
	function exportAppSnap(){
		var selections = $("#dg4").datagrid("getSelections");
		if(selections.length > 1){
			jAlert("请选择一条记录！","warning");
			return
		}
		var dealNo = selections[0].BATCH_NO;
		$.messager.progress({text:"数据处理中..."});
		$('#download').attr('src',"cardapply/cardApplyAction!exportAppSnapDetail.action?rows=65530&batchNo=" + dealNo);
		startCycle();
	}
</script>
<n:initpage title="人员进行批量申领换卡操作！<span style='color:red'>注意</span>：当客户存在一张非注销的【全功能卡】时才能进行批量换卡！">
  	<n:center>
		<div id="tb">
			<form id="searchConts">
				<input name="isPhoto"   value="<%=com.erp.util.Constants.YES_NO_YES %>"     id="isPhoto"  type="hidden"/>
				<input name="cardType"  value="<%=com.erp.util.Constants.CARD_TYPE_SMZK %>" id="cardType" type="hidden"/>
				<input name="isBatchHf" value="<%=com.erp.util.Constants.YES_NO_YES %>"     id="isBatchHf" type="hidden"/>
		        <table class="tablegrid">
					...
					<tr>
						...
						<td class="tableright" style="width:17%"><input id="isJudgeSbState" type="text" class="textinput" name="isJudgeSbState"/></td>
  		...
  		<table id="dg" title="批量申领换卡"></table>
  		...
  		<div id="dia3" >
        	<div id="tb3">
        		<span style="padding:0 10px; display:inline-block;">
  				...
  				<a href="javascript:void(0);" class="easyui-linkbutton" data-options="plain:false,iconCls:'icon-export'" onclick="exportAppSnap()">导出</a>
        	</div>
