        ...
        $.messager.confirm("系统消息",finalconfirmmsg,function(is){
            if(is){
                ...
                if(dealNull($("#rsvTwo").val()) != "09"){
                    $.post("cardService/cardServiceAction!saveCardAppOpenOrClose.action", $("#form").serialize() + "&queryType=0", function (data, status){
                        if (status == "success"){
                            if (data.status == "0"){
                    ...
                    $.post("cardService/cardServiceAction!saveCardAppOpenOrCloseHjl.action",$("#form").serialize() + "&queryType=0",function(data1, status1){
                        if(status1 == "success"){
                            if(data1.status == "0"){
                                $("#dealNo").val(data1.dealNo);
                                isNeedAutoConfirmOrCancel = true;
                                var retMsg = wirtecard_consume_zxc($("#cardNo").val(),data1.writecarddata);
...
<n:initpage title="卡片进行应用开通操作，<span style='color:red;'>注意：</span>只有卡状态处于【正常】状态的卡才能进行应用开通！">
    <n:center>
