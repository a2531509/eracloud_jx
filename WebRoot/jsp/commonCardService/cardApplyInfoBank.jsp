...
<script type="text/javascript">
	...
	var brchId = "<%=Constants.getCurrentActionLog().getBrchId()%>";
	$(function(){
		createCustomSelect({
			...
			text:"bank_name",
			table:"Base_Bank t",
			where:"bank_state = '0' and exists (select 1 from branch_bank where bank_id = t.bank_id and brch_id = '" + brchId + "')",
			...
			loadFilter:function(data){
				...
				createRecvBranch({
					...
					onLoadSuccess:function(){
						var a = $("#cardApplyInfoLkBrchId").combotree("tree").tree("find", brchId);
						if(a){
							$("#cardApplyInfoLkBrchId").combotree("setValue", brchId);
						}
					}
				});
	...
	function cardApplyInfoToApply(){
		...
		if(!$("#cardApplyInfoLkBrchId").combotree("getValue")){
			$.messager.alert("系统消息","领卡网点为空或当前网点不是领卡网点！","warning");
			return;
		}
