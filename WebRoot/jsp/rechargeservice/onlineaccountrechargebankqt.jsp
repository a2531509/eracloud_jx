...
<head>
<base href="<%=basePath%>">
<title>${ACC_KIND_NAME_LJ }圈提</title>
...
<script type="text/javascript">
		...
		$.post("cardService/cardServiceAction!getCardAndPersonInfo.action","cardNo=" + $("#cardNo").val(),function(data,status){
			if(status == "success"){
				...
				if(dealNull(data.card.cardNo).length == 0){
					$.messager.alert("系统消息","验证卡片信息发生错误：卡号信息不存在，该卡不能进行圈提。","error",function(){
						window.history.go(0);
					});
				}else{
					$.messager.progress({text : "正在加载银行卡信息,请稍后..."});
					$.post("recharge/rechargeAction!getCardBindBankInfo.action",{cardNo: $("#cardNo").val(),paySource:"1"},function(data,status){
						if(status == "success"){
							if(data.status == "0"){
	...
	function tijiao(){
		if(currentCard != $("#cardNo").val()){
			$.messager.alert("系统消息","卡号已被修改，请重新进行读卡或查询，再进行圈提！","warning");
			return;
		}
		if($("#cardNo").val().replace(/\s/g,"") == "" || $("#cardAmt").val().replace(/\s/g,"") == ""){
			$.messager.alert("系统消息","请先进行读卡或查询以获取圈提卡号和当前账户余额信息！","warning");
			return;
		}
		if($("#cardStateHidden").val() != "1"){
			$.messager.alert("系统消息","当前卡片状态不正常，不能进行${ACC_KIND_NAME_LJ }进行圈提！","warning");
			return;
		}
		if(!$("#pwd").val()){
		...
		if($("#amt").val().replace(/\s/g,"") == ""){
			$.messager.alert("系统消息","请输入${ACC_KIND_NAME_LJ }圈提金额！","warning",function(){
				$("#amt").focus();
			});
			return;
		}
		if($("#amt2").val() == ""){
			$.messager.alert("系统消息","请输入${ACC_KIND_NAME_LJ }确认圈提金额！","warning",function(){
				$("#amt2").focus();
			});
			return;
		}
		var exp = /^\d+(\.?\d{1,2})?$/g;
		if(!exp.test($("#amt").val())){
			$.messager.alert("系统消息","圈提金额格式不正确，请重新输入！","warning",function(){
				$("#amt").val("");
				$("#amt").focus();
			});
			return;
		...
		if(isNaN($("#amt").val())){
			$.messager.alert("系统消息","圈提金额格式不正确，请重新进行输入！","warning",function(){
				$("#amt").val("");
				$("#amt").focus();
			});
			return;
		...
		if($("#amt").val() != $("#amt2").val()){
			$.messager.alert("系统消息","${ACC_KIND_NAME_LJ }圈提金额和确认圈提金额不一致，请重新输入！","warning",function(){
				$("#amt2").val("");
				$("#amt2").focus();
			});
			return;
		...
		if(parseFloat($("#amt").val()) > parseFloat($("#avaiQcLimitAmt").val())){
			jAlert("圈提金额不能大于今日可用圈提金额，请重新进行输入！","warning",function(){
				$("#amt").val("");
				$("#amt").focus();
			});
			return;
		...
		if(parseFloat($("#amt").val()) <= 0){
			$.messager.alert("系统消息","${ACC_KIND_NAME_LJ }圈提金额必须大于0！","warning",function(){
				$("#amt").val("");
				$("#amt").focus();
			});
			return;
		}
		$.messager.confirm("系统消息","您确定要为卡号为【" + $("#cardNo").val() + "】的卡片圈提<" + $("#amt").val() + ">元？", function(is) {
			if(is){
				$.messager.progress({text : "正在进行圈提，请稍后...."});
				$.post("recharge/rechargeAction!saveZjzzToBankQt.action",{cardNo:$("#cardNo").val(),amt:$("#amt").val(),pwd:$("#pwd").val()},function(data,status){
					if(status == "success"){
						if(data.status != "0"){
							$.messager.progress("close");
							...
							if(data.isReminder == "0") {
								$.messager.alert("系统消息",data.reminderMsg,"info",function(){
									showReport("${ACC_KIND_NAME_LJ }圈提",data.dealNo,function(){
										window.history.go(0);
									});
								});
							} else {
						...
						$.messager.progress("close");
						$.messager.alert("系统消息","${ACC_KIND_NAME_LJ }圈提请求出现错误，请重试！","error",function(){
							window.history.go(0);
						});
					}
				},"json").error(function(){
...
<body class="easyui-layout" data-options="fit:true">
  	<div data-options="region:'north',border:false" title="" style="height:auto;overflow: hidden;">
			...
			<span class="badge">提示</span><span>在此你可以对卡片进行<span class="label-info"><strong>银行卡圈提操作，<span style='color:red'>注意</span>：圈提金额不能超过可圈提额度！</strong></span></span>
		</div>
	</div>
	<div data-options="region:'center',border:true" style="height:50px;margin:0px;width:auto;border-left:none;border-bottom:none;">
		<form id="form" method="post">
	  	...
	  	<div id="tb" style="padding:2px 0;" class="easyui-panel datagrid-toolbar" data-options="cache:false,border:false,fit:true,tools:'#toolspanel'" title="银行卡圈提">
			<table cellpadding="0" cellspacing="0" id="toolpanel" style="width:100%">
				...
				<tr>
					<td colspan="4"><h3 class="subtitle">银行卡信息</h3></td>
				</tr>
				<tr>
				...
				<tr>
					<td colspan="4"><h3 class="subtitle">圈提信息</h3></td>
				</tr>
				<tr>
					<td align="right" class="label_left">圈提限额：</td>
					...
					<td align="right" class="label_left">今日已圈提金额：</td>
				...
				<tr>
					<td align="right" class="label_left">今日已圈提笔数：</td>
					...
					<td align="right" class="label_left">今日可圈提额度：</td>
				...
				<tr>
					<td align="right" class="label_left">圈提金额：</td>
					...
					<td align="right" class="label_left">确定圈提金额：</td>
					<td class="label_right">
						...
						<shiro:hasPermission name="OnlineRechargeSave">
							<a  data-options="plain:false,iconCls:'icon-save'" href="javascript:void(0);" class="easyui-linkbutton"  onclick="tijiao()">确定圈提</a>
						</shiro:hasPermission>
					</td>
				</tr>
			</table>
