...
<script type="text/javascript">
	...
	$(function() {
		...
		createSysOrg({id:"inOrgId",isJudgePermission:true},"inBrchId","inUserId");
		$dg = createDataGrid({
			id:"dg",
			toolbar:"#tb",
			singleSelect:false,
	...
	function stockDeliveryCancel(){
		...
		if(selects.length > 0){
			...
			$.messager.confirm("系统消息","您确定要取消勾选的库存配送记录信息吗？<br><span style='color:red'>提示：批量操作请当心，已勾选" + selects.length + "条记录</span",function(m) {
				if(m){
					$.messager.progress({text : '正在进行库存配送退库，请稍后....'});
					stkSerNos = stkSerNos.substring(0,stkSerNos.length - 1);
					$.post("stockManage/stockManageAction!saveStockDeliveryCancel.action",{taskIds:stkSerNos},function(data,status){
						...
						if(status == "success"){
							...
							$.messager.alert("系统消息","库存配送取消发生错误，请重试！","error");
						}
					},"json");
				}
			});
		}else{
			$.messager.alert("系统消息","请至少选择一条配送记录进行取消","error");
		}
	}
</script>
<n:initpage title="库存配送信息进行取消操作！<span style='color:red'>注意：</span>只有库存配送人且配送记录是【未确认】状态的配送记录才能进行配送取消操作！">
	<n:center>
		<div id="tb" style="padding:2px 0">
			<form id="stockDeliveryConfirm">
				<table style="width:100%" class="tablegrid">
					...
					<tr>
						<td class="tableleft">接收机构：</td>
						<td class="tableright"><input name="rec.inOrgId" id="inOrgId" class="textinput" type="text"/></td>
						...
						<td class="tableright">
							<input name="rec.inUserId" type="text" class="textinput  easyui-validatebox" id="inUserId"  style="width:174px;"/>
							...
							<a style="text-align:center;margin:0 auto;" data-options="plain:false,iconCls:'icon-save'" href="javascript:void(0);" class="easyui-linkbutton" onclick="stockDeliveryCancel()">配送取消</a>
						</td>
					</tr>
				</table>
			</form>
