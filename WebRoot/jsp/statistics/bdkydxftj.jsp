...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $grid = createDataGrid({
            id:"dg",
            url:"unionManage/UnionManageAction!queryBdkYdConsumeStat.action",
            pagination:true,
            rownumbers:true,
            border:false,
            striped:true,
            ...
            columns:[
                [
                    ...
                    {field:"SETTLE_DATE",rowspan:2,title:"互联结算日期",sortable:true,align:"center"},
                    {field:"CLR_DATE",rowspan:2,title:"本地清分日期",sortable:true,align:"center"},
    ...
    function startDetect(){
        commonDwr.isDownloadComplete("queryBdkYdConsumeStatExport",function(data){
    ...
    function exportFile(){
        if($("#beginTime").val() > $("#beginTime").val()){
            jAlert("互联结算起始日期不能大于结束日期！");
            return;
        } else if($("#clrBeginTime").val() > $("#clrEndTime").val()){
            jAlert("本地清分起始日期不能大于结束日期！");
            return;
        }
        jConfirm("您确定要进行导出吗？",function(){
            $.messager.progress({text:"正在进行导出,请稍候..."});
            $("#searchConts").get(0).action = "unionManage/UnionManageAction!queryBdkYdConsumeStatExport.action";
            $("#searchConts").get(0).submit();
            startCycle();
        });
    }
</script>
<n:initpage title="互联互通本地卡在异地消费进行结算统计！">
    <n:center>
        <div id="tb">
            <form id="searchConts">
                <table class="tablegrid">
                    <tr>
                        <td class="tableleft">互联结算起始日期：</td>
                        ...
                        <td class="tableleft">互联结算结束日期：</td>
