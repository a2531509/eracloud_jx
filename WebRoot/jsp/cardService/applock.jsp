  ...
  <head>
	...
	<script type="text/javascript">
		...
		$(function(){
			...
			$dg = $("#dg");
			$grid=$dg.datagrid({
				url:"cardService/cardServiceAction!toAppLockQuery.action",
				fit:true,
				pagination:false,
				rownumbers:true,
				border:false,
			...
			if(row){
				if(cardinfo){
					if(dealNull(cardinfo['use_Version']).length == 0){
						$.messager.alert("系统消息","该卡已是【锁定】状态无需重复锁定！","error");
						return;
					}
				}
				$.messager.confirm('系统消息','您确定要对【' + row.NAME + '】卡号为【' + row.CARD_NO + '】的卡进行卡片应用锁定吗？<div style="color:red;margin-left:42px;">提示：锁定后将不能进行任何交易' + '</div>',function(is){
					if(is){
						$.messager.progress({text:'数据处理中，请稍后....'});
						$.post('cardService/cardServiceAction!saveAppLockHjl.action',$('#form').serialize() + '&rec.cardNo=' + row.CARD_NO + "&cardAmt=" + $("#cardAmt").val(),function(data,status){
							if(status == "success"){
								if(data.status == '0'){
									if(lockcard()){//
										$("#curState").combobox("setValue","1");
										cardinfo = getcardinfo();
										lockcardconfirm(data.dealNo);
									}else{
										lockcardcancel(data.dealNo);
									}
								}else{
									$.messager.progress('close');
									$.messager.alert("系统消息","卡片应用锁定发生错误：" + data.msg,"error");
								}
							}else{
								$.messager.progress('close');
								$.messager.alert("系统消息","卡片应用锁定发生错误：请拿起并重新放置好卡片重新进行操作！","error");
							}
						},'json');
					}
				});
			}else{
				$.messager.alert("系统消息","请勾选一条记录信息进行卡片应用锁定","error");
			}
		}
		function lockcard(){
			return CardAppBlock1001();
		}
		function lockcardconfirm(dealNo){
			$.post('cardService/cardServiceAction!saveAppLockHjlConfirm.action',{'rec.dealNo':dealNo},function(data,status){
				$.messager.progress('close');
				/* $.messager.progress('close');*/
				$.messager.alert('系统消息',"卡片应用锁定成功！","info"); 
			});
		}
		function lockcardcancel(dealNo){
			$.post('cardService/cardServiceAction!saveAppLockHjlCancel.action',{'rec.dealNo':dealNo},function(data,status){
				$.messager.progress('close');
				$.messager.alert('系统消息',"卡片应用锁定发生错误：请拿起并重新放置好卡片重新进行操作！","error");
				/* $.messager.progress('close');
...
<body class="easyui-layout" data-options="fit:true">
  	<div data-options="region:'north',border:false" title="" style="overflow: hidden; padding: 0px;">
		<div class="well well-small datagrid-toolbar">
			...
			<span>在此你可以对<span class="label-info"><strong>卡片进行卡片应用锁定操作！<span style="color:red;">注意：</span>卡片锁定后将不能进行任何交易。</strong></span></span>
		</div>
	</div>
					...
					<table class="tablegrid">
						<tr>
							...
							<td class="tableright">
									...
									<shiro:hasPermission name="saveAppLockHjl">
										<a  data-options="plain:false,iconCls:'icon-save'" href="javascript:void(0);"  class="easyui-linkbutton"  onclick="toSaveLock();">应用锁定</a>
									</shiro:hasPermission>
							</td>
						</tr>
					</table>
