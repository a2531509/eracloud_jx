...
<head>
	...
	<script type="text/javascript">
		$(function(){
			$("#outCardNo").validatebox({required:true,invalidMessage:"请读卡以获取转出卡信息<br/><span style=\"color:red\">提示：卡号为有效的20位数字或字母组成</span>",missingMessage:"请读卡以获取转出卡信息<br/><span style=\"color:red;\">提示：卡号为有效的20位数字或字母组成</span>"});
			$("#outAccBal").validatebox({required:true,invalidMessage:"请读卡以获取转出卡账户信息<br/><span style=\"color:red;\">提示：卡号为有效的20位数字或字母组成</span>",missingMessage:"请读卡以获取转出卡账户信息<br/><span style=\"color:red;\">提示：卡号为有效的20位数字或字母组成</span>"});
			$("#inCardNo").validatebox({required:true,invalidMessage:"请读卡以获取转入卡信息<br/><span style=\"color:red;\">提示：卡号为有效的20位数字或字母组成</span>",missingMessage:"请读卡以获取转入卡信息<br/><span style=\"color:red;\">提示：卡号为有效的20位数字或字母组成</span>"});
			$("#inAccBal").validatebox({required:true,invalidMessage:"请读卡以获取转入卡账户信息<br/><span style=\"color:red;\">提示：卡号为有效的20位数字或字母组成</span>",missingMessage:"请读卡以获取转入卡账户信息<br/><span style=\"color:red;\">提示：卡号为有效的20位数字或字母组成</span>"});
		...
		function readcard(type){
			...
			if(type == "0"){
				...
				$("#amt").val(Number(cardinfo["wallet_Amt"]).div100());
				$("#confirmAmt").val(Number(cardinfo["wallet_Amt"]).div100());
			}else if(type == "1"){
		...
		function confirmTransfer() {
			...
			if(dealNull($("#outCardStateHidden").val()) != "<%=com.erp.util.Constants.CARD_STATE_ZX%>") {
				jAlert("转出卡卡状态不是注销状态，不能进行转出操作！");
				return;
			}
		...
		function writeAshRecord(){
			$.post("recharge/rechargeAction!hkzzGoodCard.action", 
			{
...
<body class="easyui-layout" data-options="fit: true">
	<div style="height: auto; overflow: hidden;" data-options="region: 'north', border: false">
 		<div class="well well-small datagrid-toolbar" style="margin-left: 0px; margin-right: 0px; margin-top: 2px; margin-bottom: 2px;">
			...
			<span>
				在此你可以对卡片进行<span class="label-info"><strong>好卡换卡转钱包</strong>操作</span>
			</span>
		</div>
	</div>
	<div data-options="region:'center',border:true,fit:true" style="margin:0px;width:auto;height:auto" title="${ACC_KIND_NAME_QB }转${ACC_KIND_NAME_QB }" class="datagrid-toolbar">
		...
		<div>
			...
			<table class="tablegrid" style="width:100%;">
				<tr>
					<th width="23%" class="tableleft">代理人证件类型：</th>
					<td width="18%" class="tableright"><input id="agtCertType" type="text" class="textinput easyui-validatebox" name="rec.agtCertType" value="1" style="width:174px;"/> </td>
					<th width="18%" class="tableleft">代理人证件号码：</th>
					<td width="39%" class="tableright"><input name="rec.agtCertNo"  class="textinput easyui-validatebox" id="agtCertNo" type="text" validtype="idcard" maxlength="18"/></td>
				</tr>
				<tr>
					...
					<td class="tableright">
						<input name="rec.agtTelNo" id="agtTelNo" type="text" class="textinput easyui-validatebox" validtype="mobile" maxlength="11" />
		...
		<div style="width:100%;height:auto;border:none;">
			<div id="tb2" style="padding:2px 0;background-color:rgb(245,245,245);overflow:hidden;" class="easyui-panel" data-options="fit:true,cache:false,border:false,tools:'#zzMsg'">
				...
				<table cellpadding="0" cellspacing="0" id="toolpanel2" style="width:100%" class="tablegrid" id="zzMsg">
					<tr>
						<td width="23%" class="tableleft" style="display: none">转账金额：</td>
						<td width="18%" class="tableright"  style="display: none"><input id="amt" type="text" readonly="readonly" class="easyui-validatebox textinput" name="amt" data-options="required:true,validType:'number',tipPosition:'right',invalidMessage:'转账金额不可大于转出卡的账户余额',missingMessage:'转账金额不可大于转出卡的账户余额'" onkeydown="validRmb(this)" onkeyup="validRmb(this)"/></td>
						<td width="59%" class="tableleft">转账金额：</td>
						<td width="39%" class="tableright">
							<input style="display:inline-block;vertical-align: baseline;" id="confirmAmt" readonly="readonly" type="text" class="textinput easyui-validatebox" name="confirmAmt" data-options="required:true,validType:'number',tipPosition:'left',invalidMessage:'转账金额不可大于转出卡的账户余额',missingMessage:'转账金额不可大于转出卡的账户余额'" onkeydown="validRmb(this)" onkeyup="validRmb(this)"/>
							<a data-options="plain:false,iconCls:'icon-ok'" href="javascript:void(0);" class="easyui-linkbutton"  onclick="confirmTransfer()">转账</a>
						</td>
					</tr>
				</table>
			</div>
