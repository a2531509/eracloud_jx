...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $cardinfo = createDataGrid({
            id:"cardinfo",
            url:"cardService/cardServiceAction!toFjmkBhkZzQuery.action",
            fit:true,
            pagination:false,
            rownumbers:true,
            border:false,
            ...
            columns:[[
                ...
                {field:"CARDTYPE",title:"卡类型",sortable:true,width:parseInt($(this).width() * 0.1)},
                ...
                {field:"ISGOODCARD",title:"钱包账户退还方式",sortable:false},
        ...
        if(curcard){
            ...
            if(allacc && allacc.length > 0){
                for(var i = 0;i < allacc.length;i++){
                    if(allacc[i].ACC_KIND == "01" && curcard.IS_GOOD_CARD == "0" && allacc[i].BAL_RSLT_FLAG == "0"){//RSV_ONE
                        tempFee = (Number(parseFloat(tempFee).toFixed(2)) + Number(parseFloat(curcard.PRV_BAL).toFixed(2)));
                    }else if(allacc[i].BAL_RSLT_FLAG == "0" && curcard.IS_GOOD_CARD != "0"){
        ...
        if(curcard){
            ...
            $.messager.confirm("系统消息","您确定要进行换卡转钱包吗？" + "<br/>转账总金额：" + $("#totalAmt").val() ,function(is){
                if(is){
                    ...
                    $.post("cardService/cardServiceAction!saveFjmkBhkZzAshHjl.action",$("form").serialize()+ "&rec.cardNo=" + $("#cardNo").val() + "&cardAmt=" + $("#cardAmt").val(),function(data,status){
                        if(status == "success"){
                            if(data.status == "0"){
                                $.messager.progress("close");
        ...
        <div id="tb">
            <table class="tablegrid">
                ...
                <tr>
                    ...
                    <td class="tableright">
                        <a data-options="plain:false,iconCls:'icon-readCard'" href="javascript:void(0);" class="easyui-linkbutton" onclick="readCard()">读新卡</a>
