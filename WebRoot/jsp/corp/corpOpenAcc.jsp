...
<script type="text/javascript">
	$(function() {
		...
		$("#dg").datagrid({
			...
			frozenColumns : [ [
				...
				{field:"OPEN_FLAG", title:"开户状态", sortable:true, width : parseInt($(this).width() * 0.1), formatter:function(value){
					if(value == 1){
						return "<span style='color:green'>已开户</span>";
					} else {
						return "<span style='color:orange'>未开户</span>";
					}
				}}
			] ],
			columns : [[
	...
	function corpOpenAcc(){
		...
		$.post("corpManager/corpManagerAction!corpOpenAcc.action", {customerId:selection[0].CUSTOMER_ID}, function(data){
			if(data.status == "1"){
				$.messager.alert("消息提示", data.errMsg, "error");
			} else {
				$.messager.alert("消息提示", "单位开户成功", "info");
			}
		}, "json");
	}
</script>
	...
	<div data-options="region:'north',border:false"
		...
		<div class="well well-small datagrid-toolbar" style="margin: 2px 0">
			<span class="badge">提示</span> <span>在此你可以对<span
				class="label-info"><strong>审核通过的单位</strong></span>进行<span
				class="label-info"><strong>开户处理</strong></span></span>
		</div>
	</div>
			...
			<table class="tablegrid" cellpadding="0" cellspacing="0" width="100%">
				...
				<tr>
					...
					<td class="tableright" colspan="2" style="padding-left: 15px;"><a
						...
						href="javascript:void(0);" class="easyui-linkbutton"
						iconCls="icon-orgOpenAcc" onclick="corpOpenAcc()">单位开户</a></td>
				</tr>
			</table>
		</div>
