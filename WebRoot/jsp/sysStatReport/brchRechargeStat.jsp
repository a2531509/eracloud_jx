...
<script type="text/javascript"> 
	...
	$(function() {
		...
		$dg = $("#dg");
		$grid = $dg.datagrid({
			url : "sysReportQuery/sysReportQueryAction!queryBrchRechgStatRt.action",
			pagination:true,
			rownumbers:true,
			border:false,
			striped:true,
			...
			frozenColumns:[
				[
					...
					{field:'FULL_NAME',title:'受理点',rowspan:3,align:'center',width:parseInt($(this).width()*0.1)},
				...
				[
					...
					{field:'LJ_TOT_NUM',title:'笔数',align:'center', formatter:function(value, row, index){
						row.LJ_TOT_NUM = parseInt(row.XJ_RECHARGE_LJ_NUM) - parseInt(row.XJ_RECHARGE_LJ_CX_NUM) + parseInt(row.BNAK_RECHARGE_LJ_NUM) - parseInt(row.BNAK_RECHARGE_LJ_CX_NUM) + parseInt(row.CORP_RECHARGE_LJ_NUM);
						return row.LJ_TOT_NUM;
					}},
					{field:'LJ_TOT_AMT',title:'金额',align:'center', formatter:function(value,row,index){
						row.LJ_TOT_AMT = parseFloat(row.XJ_RECHARGE_LJ_AMT) + parseFloat(row.XJ_RECHARGE_LJ_CX_AMT) + parseFloat(row.BNAK_RECHARGE_LJ_AMT) + parseFloat(row.BNAK_RECHARGE_LJ_CX_AMT) + parseFloat(row.CORP_RECHARGE_LJ_AMT);
						return fmoney((row.LJ_TOT_AMT/100).toFixed(2));
					}}
	      		]
               ],
		columns :[
				...
				[
					...
					{field:'TJ_TOT_NUM',title:'笔数',align:'center', formatter:function(value, row, index){
						row.TJ_TOT_NUM = parseInt(row.XJ_RECHARGE_TJ_NUM) - parseInt(row.XJ_RECHARGE_TJ_CX_NUM) + parseInt(row.BNAK_RECHARGE_TJ_NUM) - parseInt(row.BNAK_RECHARGE_TJ_CX_NUM);
						return row.TJ_TOT_NUM;
					}},
					{field:'TJ_TOT_AMT',title:'金额',align:'center',formatter:function(value,row,index){
						row.TJ_TOT_AMT = parseFloat(row.XJ_RECHARGE_TJ_AMT) + parseFloat(row.XJ_RECHARGE_TJ_CX_AMT) + parseFloat(row.BNAK_RECHARGE_TJ_AMT) + parseFloat(row.BNAK_RECHARGE_TJ_CX_AMT);
						return fmoney((row.TJ_TOT_AMT/100).toFixed(2));
					}},
					...
					{field:'TOTAL_NUM',title:'合计笔数',align:'center',sortable:false,width:parseInt($(this).width()*0.05), formatter:function(v, row, i){
						row.TOTAL_NUM = parseInt(row.XJ_RECHARGE_LJ_NUM) - parseInt(row.XJ_RECHARGE_LJ_CX_NUM) + parseInt(row.BNAK_RECHARGE_LJ_NUM) - parseInt(row.BNAK_RECHARGE_LJ_CX_NUM) + parseInt(row.CORP_RECHARGE_LJ_NUM)
									  + parseInt(row.XJ_RECHARGE_TJ_NUM) - parseInt(row.XJ_RECHARGE_TJ_CX_NUM) + parseInt(row.BNAK_RECHARGE_TJ_NUM) - parseInt(row.BNAK_RECHARGE_TJ_CX_NUM) + parseInt(row.BATCH_RECHARGE_UR_NUM);
						return row.TOTAL_NUM;
					}},
					{field:'TOTAL_AMT',title:'合计金额',align:'center',sortable:false,width:180,formatter:function(value,row,index){
						row.TOTAL_AMT = row.LJ_TOT_AMT = parseFloat(row.XJ_RECHARGE_LJ_AMT) + parseFloat(row.XJ_RECHARGE_LJ_CX_AMT) + parseFloat(row.BNAK_RECHARGE_LJ_AMT) + parseFloat(row.BNAK_RECHARGE_LJ_CX_AMT) + parseFloat(row.CORP_RECHARGE_LJ_AMT)
									+ parseFloat(row.XJ_RECHARGE_TJ_AMT) + parseFloat(row.XJ_RECHARGE_TJ_CX_AMT) + parseFloat(row.BNAK_RECHARGE_TJ_AMT) + parseFloat(row.BNAK_RECHARGE_TJ_CX_AMT) + parseFloat(row.BATCH_RECHARGE_UR_AMT);
						return fmoney((row.TOTAL_AMT/100).toFixed(2));
					}}
	      		]
			],
			toolbar:'#tb',
			...
			onLoadSuccess:function(data){
            	  ...
            	  if(data.status != 0){
            		 $.messager.alert('系统消息',data.errMsg,'error');
            		  ...
            		  $dg.datagrid("autoMergeCells",['FULL_NAME']);
            	  }
            	  updateFooter();
            },
            onSelect:function(){
	...
	function execelRechargeRep(){
		...
		if (rows.length>0) {
			...
			$.post("sysReportQuery/sysReportQueryAction!setExportParams.action", {id:id}, function(data){
				if(data.status == 0){
					$('#downloadcsv').attr('src','sysReportQuery/sysReportQueryAction!exportToExcelRealTimeRechargeMsg.action?rows=2000&' + $("#rechargeMsgFrom").serialize());
				} else {
  		...
  		<div id="tb" style="padding:2px 0">
  			<form id="rechargeMsgFrom">
				<table cellpadding="0" cellspacing="0" width="100%" class="tablegrid">
					...
					<tr>
						<td class="tableleft">交易起始日期：</td>
						<td class="tableright"><input  id="startDate" type="text" name="startDate" class="Wdate textinput" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',qsEnabled:false,maxDate:'%y-%M-%d %H:%m:%s',minDate:'%y-%M-%d 00:00:00'})" value="<%=DateUtil.formatDate(new Date(), "yyyy-MM-dd") + " 00:00:00"%>"/></td>
						<td class="tableleft">交易结束日期：</td>
						<td class="tableright"><input id="endDate" type="text"  name="endDate" class="Wdate textinput"  onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',qsEnabled:false,minDate:'%y-%M-%d 00:00:00',maxDate:'%y-%M-%d %H:%m:%s'})" value="<%=DateUtil.formatDate(new Date())%>"/>
						</td>
