...
<script type="text/javascript">
	...
	$(function(){
		...
		$grid = $("#dg").datagrid({
			url:"sysReportQuery/sysReportQueryAction!querySysPayReadyRp2.action",
			pagination:true,
			rownumbers:true,
			border:false,
			striped:true,
	...
	function query(){
		...
		$grid.datagrid("load",{
			queryType:"0",
			startDate:dealNull($("#startDate").val()),
			endDate:dealNull($("#endDate").val())
		});
	}
	
	function exportExcel(){
		$.messager.progress({text:"数据处理中, 请稍候..."});
		$("#download-frame").attr("src", "sysReportQuery/sysReportQueryAction!exportPayReady.action?startDate=" + $("#startDate").val() + "&endDate=" + $("#endDate").val());
		$.messager.progress("close");
	}
</script>
<n:initpage title="系统备份金账户统计报表">
	<n:center>
		<div id="tb">
			<table class="tablegrid">
					<tr>
						<td class="tableleft" style="width:8%">起始时间：</td>
						<td class="tableright" style="width:17%"><input id="startDate" name="startDate" type="text" class="textinput"   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',qsEnabled:false})"/></td>
						<td class="tableleft" style="width:8%">结束时间：</td>
						<td class="tableright" style="width:17%"><input id="endDate"   name="endDate"   type="text" class="textinput"   onclick="WdatePicker({dateFmt:'yyyy-MM-dd',qsEnabled:false})"/></td>
