...
$(function(){
	...
	$gridview=$dgview.datagrid({
		url:"clrDeal/clrDealAction!findAllCheckBillList.action",
		pagination:true,
		rownumbers:true,
		border:false,
		striped:true,
...
function autoCom(){
	...
	$("#coOrgId2").autocomplete({
	    ...
	    source: function(request,response){
		    $.post('cooperationAgencyManager/cooperationAgencyAction!initAutoComplete.action',{"co.coOrgId":$("#coOrgId2").val(),"queryType":"1","initCorpType":"1"},function(data){
...
function autoComByName(){
	...
	$("#coOrgName2").autocomplete({
	    source:function(request,response){
	        $.post('cooperationAgencyManager/cooperationAgencyAction!initAutoComplete.action',{"co.coOrgName":$("#coOrgName2").val(),"queryType":"0","initCorpType":"1"},function(data){
...
function dealdzcorepair(){
	...
	if(row){
		...
		$.ajax({   
	        ...
	        type:"POST",  
	        dataType:"json",
	        url:"clrDeal/clrDealAction!dealdzcorepair.action",   
...
function dealdzorgcancel(){
	...
	if(row){
		...
		$.ajax({   
	        ...
	        type:"POST",  
	        dataType:"json",
	        url:"clrDeal/clrDealAction!dealdzorgcancel.action",   
	        data:{checkListId:row.ID},   
	        error:function(jqXHR, textStatus, errorThrown){   
	            if(textStatus=="timeout"){
	            	$.messager.progress('close');
...
function dealdzorgadd(){
	...
	if(row){
		...
		$.ajax({   
	        ...
	        type:"POST",  
	        dataType:"json",
	        url:"clrDeal/clrDealAction!dealdzorgadd.action",   
	        data:{checkListId:row.ID},   
	        error:function(jqXHR, textStatus, errorThrown){   
	            if(textStatus=="timeout"){
	            	$.messager.progress('close');
...
function dealdzdeletemx(){
	...
	if(row){
		...
		$.ajax({ 
	        ...
	        type:"POST",  
	        dataType:"json",
	        url:"clrDeal/clrDealAction!dealdzdeletemx.action",   
	        data:{checkListId:row.ID},   
	        error:function(jqXHR, textStatus, errorThrown){   
	            if(textStatus=="timeout"){
	            	$.messager.progress('close');
