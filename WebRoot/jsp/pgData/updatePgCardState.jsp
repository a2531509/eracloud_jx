...
<script type="text/javascript"> 
	...
	function query() {
		...
		params.queryType = 0;
		params["rec.dealCode"] = "<%=DealCode.UPDATE_ST_CARD_STATE%>";
	...
	function updateCardState(){
		...
		var cardState = $("#cardState").combobox("getValue");
		if(!cardNo){
			$.messager.alert("系统消息","卡号不能为空！","error");
			return;
		} else if(!cardState){
			$.messager.alert("系统消息","变更状态不能为空！","error");
			return;
		}
		$.messager.confirm("系统消息","你确定变更卡号为【" + cardNo + "】的卡片到【" + $("#cardState").combobox("getText") + "】状态？", function(r){
			if(r){
				$.messager.progress({text:"数据处理中..."});
				$.post("pgData/pgDataAction!updateCardState.action", {cardNo:cardNo, cardStateAfterChange:cardState}, function(data){
					$.messager.progress("close");
					if (data.status == 1) {
			...
			<form id="searchConts">
				<table class="tablegrid" style="width:auto">
					<tr>
						...
						<td class="tableleft" style="padding: 0 10px">变更状态：</td>
						<td class="tableright"style="padding: 0 10px"><input  id="cardState" type="text" class="textinput" name="cardState" /></td>
						<td class="tableleft" colspan="8" style="padding: 0 10px">
							<a style="text-align:center;margin:0 auto;" data-options="plain:false,iconCls:'icon-save'" href="javascript:void(0);" class="easyui-linkbutton" onclick="updateCardState()">变更状态</a>
