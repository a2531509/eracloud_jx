...
<script type="text/javascript"> 
	...
	$(function(){
		...
		$grid = createDataGrid({
			...
			fit:true,pagination:false,rownumbers:true,border:false,striped:true,fitColumns:true,
			autoRowHeight:true,scrollbarSize:0,singleSelect:true,
			columns:[[
		...
		$.post("cardService/cardServiceAction!getCardAndPersonInfo.action","cardNo=" + $("#cardNo").val(),function(data,status){
			...
			if(status == "success"){
				if(dealNull(data.card.cardNo).length == 0){
					$.messager.alert("系统消息","验证卡片错误，卡号信息不存在，该卡不能进行${ACC_KIND_NAME_LJ }密码重置。","error",function(){
						window.history.go(0);
					});
				}else{
	...
	function submitForm(){
		...
		if(!curRow){
			$.messager.alert("系统消息","请至少选择一条记录进行${ACC_KIND_NAME_LJ }支付密码重置！","error");
			return;
		}
		...
		$.messager.confirm("系统消息","您确定要重置卡号为【" + curRow.CARD_NO + "】的${ACC_KIND_NAME_LJ }支付密码吗？",function(is){
			if(is){
				...
				$.post("pwdservice/pwdserviceAction!savePayPwdReset.action",reqpara,function(data,status){
					$.messager.progress("close");
					if(status == "success"){
						if(dealNull(data.status) == "0"){
							showReport("${ACC_KIND_NAME_LJ }支付密码重置",data.dealNo,function(){
								window.history.go(0);
							});
						}else{
...
<n:initpage title="个人${ACC_KIND_NAME_LJ }支付密码进行重置操作！<span style='color:red;font-weight:700'>注意：</span>如果原密码已存在，请进行${ACC_KIND_NAME_LJ }支付密码修改！">
	<n:center>
		<div id="tb">
			<table class="tablegrid">
				<tr>
					...
					<td style="padding-left:2px">
						...
						<shiro:hasPermission name="savePayPwdReset">
	  ...
	  <div data-options="region:'south',split:false,border:true" style="height:300px;width:auto;text-align:center;overflow:hidden;border-left:none;border-bottom:none;">
	  		<div style="width:100%;overflow:hidden;height:auto;border:none;" class="panel-body" id="ko">
		  	...
		  	<div style="width:100%;overflow:hidden;height:100%;" class="datagrid-toolbar">
