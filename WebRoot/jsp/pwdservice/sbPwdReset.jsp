...
<script type="text/javascript">
	...
	function getNewPwd(){
		...
		if(!flag){
			$.messager.alert("系统消息","不能重置密码, " + errMsg,"error");
			return;
		}
	...
	function modify(){
		...
		$.messager.confirm("系统消息","您确定要重置卡号为【" + cardNo + "】的医保密码吗？",function(is){
			if(is){
				$.messager.progress({text : "正在重置密码，请不要取走卡片..."});
				if(!reSetTouchPwd(newPwd)){
					$.messager.progress("close");
					return;
				}
				$.messager.progress("close");
				...
				$.post("pwdservice/pwdserviceAction!sbPwdReset.action",reqpara,function(data,status){
						$.messager.progress("close");
						if(data.status == "0"){
							$.messager.alert("系统消息","重置密码成功", "info");
							showReport("医保密码重置",data.dealNo,function(){
								window.history.go(0);
							});
						}else{
...
<n:initpage
	title="个人医保密码</strong>进行重置操作！<span style='color:red;font-weight:700'>">
	<n:center>
		<div id="tb">
