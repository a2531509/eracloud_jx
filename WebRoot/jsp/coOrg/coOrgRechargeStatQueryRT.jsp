...
<script type="text/javascript"> 
	...
	$(function() {
		...
		$dg = $("#dg");
		$grid = $dg.datagrid({
			url : "sysReportQuery/sysReportQueryAction!queryCoOrgRechargeStatRt.action",
			pagination:true,
			rownumbers:true,
			border:false,
			striped:true,
	...
	function execelRechargeRep(){
		...
		$('#downloadcsv').attr('src','sysReportQuery/sysReportQueryAction!exportCoOrgRechargeRTMsg.action?rows=2000&coOrgIds=' + coOrgIds.substring(0, coOrgIds.length - 1) + paramStr);
	}
	
</script>
<n:initpage title="合作机构充值实时统计">
  	<n:center>
  			...
  			<form id="rechargeMsgFrom">
				<table cellpadding="0" cellspacing="0" width="100%" class="tablegrid">
					<tr>
						...
						<td class="tableright"><input id="endDate" type="text" name="endDate" class="Wdate textinput" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',qsEnabled:false,maxDate:'%y-%M-%d %H:%m:%s',minDate:'%y-%M-%d 00:00:00'})" value="<%=DateUtil.formatDate(new Date())%>"/>
						</td>
					</tr>
					<tr>
		...
		<table id ="dg" title="合作机构充值情况统计(实时)"></table>
