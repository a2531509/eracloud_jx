...
<script type="text/javascript"> 
	...
	$(function() {
		...
		$dg = $("#dg");
		$grid = $dg.datagrid({
			url : "sysReportQuery/sysReportQueryAction!queryCoOrgRechargeMsg.action",
			pagination:true,
			rownumbers:true,
			border:false,
			striped:true,
			...
			columns:[
				[
					{title:'已对账',colspan:20,align:'center'},
					{title:'未对账',colspan:20,align:'center'},
				...
				[
					...
					{title:'充值撤销',colspan:2,align:'center'},
					{title:'小计',colspan:2,align:'center'},
					{field:'WDZ_NUM',title:'笔数',align:'center', rowspan:2,width:'80px'},
				...
				[
					...
					{field:'LJ_YDZ_CZ_TOT_NUM',title:'笔数',align:'center',width:'80px',formatter:function(value,row,index){
						return row.LJ_YDZ_CZ_NUM - row.LJ_YDZ_CZCX_NUM;
					}},
					...
					// 未对账 - 联机
					{field:'LJ_WDZ_CZ_NUM',title:'笔数',align:'center',width:'80px'},
					{field:'LJ_WDZ_CZ_AMT',title:'金额',align:'center',width:'80px',formatter:function(value,row,index){
						if(value == "0"){
							return "0.00";
						}else{
							return $.foramtMoney(Number(value).div100());
					...
					{field:'LJ_WDZ_CZCX_NUM',title:'笔数',align:'center',width:'80px'},
					{field:'LJ_WDZ_CZCX_AMT',title:'金额',align:'center',width:'80px',formatter:function(value,row,index){
						if(value == "0"){
							return "0.00";
						}else{
							return $.foramtMoney(Number(value).div100());
					...
					{field:'LJ_WDZ_CZ_TOT_NUM',title:'笔数',align:'center',width:'80px',formatter:function(value,row,index){
						return row.LJ_WDZ_CZ_NUM - row.LJ_WDZ_CZCX_NUM;
					}},
					{field:'LJ_WDZ_CZ_TOT_AMT',title:'金额',align:'center',width:'80px',formatter:function(value,row,index){
						var amt = parseFloat(row.LJ_WDZ_CZ_AMT) + parseFloat(row.LJ_WDZ_CZCX_AMT);
						return $.foramtMoney(Number(amt).div100());
					}},
					// 未对账 - 钱包
					{field:'QB_WDZ_CZ_NUM',title:'笔数',align:'center', width:'80px'},
					{field:'QB_WDZ_CZ_AMT',title:'金额',align:'center', width:'80px',formatter:function(value,row,index){
						if(value == "0"){
							return "0.00";
						}else{
							return $.foramtMoney(Number(value).div100());
					...
					{field:'QB_WDZ_CZCX_NUM',title:'笔数',align:'center', width:'80px'},
					{field:'QB_WDZ_CZCX_AMT',title:'金额',align:'center', width:'80px',formatter:function(value,row,index){
						if(value == "0"){
							return "0.00";
						}else{
							return $.foramtMoney(Number(value).div100());
					...
					{field:'QB_WDZ_CZ_TOT_NUM',title:'笔数',align:'center',width:'80px',formatter:function(value,row,index){
						return row.QB_WDZ_CZ_NUM - row.QB_WDZ_CZCX_NUM;
					}},
					{field:'QB_WDZ_CZ_TOT_AMT',title:'金额',align:'center',width:'80px',formatter:function(value,row,index){
						var amt = parseFloat(row.QB_WDZ_CZ_AMT) + parseFloat(row.QB_WDZ_CZCX_AMT);
						return $.foramtMoney(Number(amt).div100());
					}},
					// 未对账 - 未登
					{field:'UR_WDZ_CZ_NUM',title:'笔数',align:'center', width:'80px'},
					{field:'UR_WDZ_CZ_AMT',title:'金额',align:'center', width:'80px',formatter:function(value,row,index){
						if(value == "0"){
							return "0.00";
						}else{
							return $.foramtMoney(Number(value).div100());
					...
					{field:'UR_WDZ_CZCX_NUM',title:'笔数',align:'center', width:'80px'},
					{field:'UR_WDZ_CZCX_AMT',title:'金额',align:'center', width:'80px',formatter:function(value,row,index){
						if(value == "0"){
							return "0.00";
						}else{
							return $.foramtMoney(Number(value).div100());
					...
					{field:'UR_WDZ_CZ_TOT_NUM',title:'笔数',align:'center',width:'80px',formatter:function(value,row,index){
						return row.UR_WDZ_CZ_NUM - row.UR_WDZ_CZCX_NUM;
					}},
					{field:'UR_WDZ_CZ_TOT_AMT',title:'金额',align:'center',width:'80px',formatter:function(value,row,index){
						var amt = parseFloat(row.UR_WDZ_CZ_AMT) + parseFloat(row.UR_WDZ_CZCX_AMT);
						return $.foramtMoney(Number(amt).div100());
					}},
					//
					{field:'TOT_NUM',title:'笔数',align:'center',width:'80px'},
	...
	function updateFooter(){
		...
		var LJ_WDZ_CZ_NUM = 0;
		var LJ_WDZ_CZ_AMT = 0;
		var LJ_WDZ_CZCX_NUM = 0;
		var LJ_WDZ_CZCX_AMT = 0;
		// 钱包
		var QB_WDZ_CZ_NUM = 0;
		var QB_WDZ_CZ_AMT = 0;
		var QB_WDZ_CZCX_NUM = 0;
		var QB_WDZ_CZCX_AMT = 0;
		// 未登
		var UR_WDZ_CZ_NUM = 0;
		var UR_WDZ_CZ_AMT = 0;
		var UR_WDZ_CZCX_NUM = 0;
		var UR_WDZ_CZCX_AMT = 0;
		// 小计
		var WDZ_NUM = 0;
		var WDZ_AMT = 0;
		...
		for(var i in selections){
			...
			//
			LJ_WDZ_CZ_NUM += isNaN(Number(row.LJ_WDZ_CZ_NUM))?0:Number(row.LJ_WDZ_CZ_NUM);
			LJ_WDZ_CZ_AMT += isNaN(Number(row.LJ_WDZ_CZ_AMT))?0:Number(row.LJ_WDZ_CZ_AMT);
			LJ_WDZ_CZCX_NUM += isNaN(Number(row.LJ_WDZ_CZCX_NUM))?0:Number(row.LJ_WDZ_CZCX_NUM);
			LJ_WDZ_CZCX_AMT += isNaN(Number(row.LJ_WDZ_CZCX_AMT))?0:Number(row.LJ_WDZ_CZCX_AMT);
			//
			QB_WDZ_CZ_NUM += isNaN(Number(row.QB_WDZ_CZ_NUM))?0:Number(row.QB_WDZ_CZ_NUM);
			QB_WDZ_CZ_AMT += isNaN(Number(row.QB_WDZ_CZ_AMT))?0:Number(row.QB_WDZ_CZ_AMT);
			QB_WDZ_CZCX_NUM += isNaN(Number(row.QB_WDZ_CZCX_NUM))?0:Number(row.QB_WDZ_CZCX_NUM);
			QB_WDZ_CZCX_AMT += isNaN(Number(row.QB_WDZ_CZCX_AMT))?0:Number(row.QB_WDZ_CZCX_AMT);
			//
			UR_WDZ_CZ_NUM += isNaN(Number(row.UR_WDZ_CZ_NUM))?0:Number(row.UR_WDZ_CZ_NUM);
			UR_WDZ_CZ_AMT += isNaN(Number(row.UR_WDZ_CZ_AMT))?0:Number(row.UR_WDZ_CZ_AMT);
			UR_WDZ_CZCX_NUM += isNaN(Number(row.UR_WDZ_CZCX_NUM))?0:Number(row.UR_WDZ_CZCX_NUM);
			UR_WDZ_CZCX_AMT += isNaN(Number(row.UR_WDZ_CZCX_AMT))?0:Number(row.UR_WDZ_CZCX_AMT);
			//
		...
		$("#dg").datagrid("reloadFooter", [{
			...
			YDZ_AMT : YDZ_AMT,
			//
			LJ_WDZ_CZ_NUM : LJ_WDZ_CZ_NUM,
			LJ_WDZ_CZ_AMT : LJ_WDZ_CZ_AMT,
			LJ_WDZ_CZCX_NUM : LJ_WDZ_CZCX_NUM,
			LJ_WDZ_CZCX_AMT : LJ_WDZ_CZCX_AMT,
			//
			QB_WDZ_CZ_NUM : QB_WDZ_CZ_NUM,
			QB_WDZ_CZ_AMT : QB_WDZ_CZ_AMT,
			QB_WDZ_CZCX_NUM : QB_WDZ_CZCX_NUM,
			QB_WDZ_CZCX_AMT : QB_WDZ_CZCX_AMT,
			//
			UR_WDZ_CZ_NUM : UR_WDZ_CZ_NUM,
			UR_WDZ_CZ_AMT : UR_WDZ_CZ_AMT,
			UR_WDZ_CZCX_NUM : UR_WDZ_CZCX_NUM,
			UR_WDZ_CZCX_AMT : UR_WDZ_CZCX_AMT,
			//
			WDZ_NUM : WDZ_NUM,
			WDZ_AMT : WDZ_AMT,
			//
	...
	function execelRechargeRep(){
		...
		$('#downloadcsv').attr('src','sysReportQuery/sysReportQueryAction!exportCoOrgRechargeMsg.action?rows=2000&coOrgIds=' + coOrgIds.substring(0, coOrgIds.length - 1) + paramStr);
	}
				...
				<table cellpadding="0" cellspacing="0" width="100%" class="tablegrid">
					...
					<tr>
						<td class="tableright" colspan="8" style="padding-left: 10px">
							...
							<a href="javascript:void(0);" class="easyui-linkbutton" iconCls="icon-reload"  plain="false" onclick="updateStat();">更新对账统计</a>
						</td>
					</tr>
				</table>
			</form>
		</div>
		<table id ="dg" title="合作机构充值情况统计"></table>
