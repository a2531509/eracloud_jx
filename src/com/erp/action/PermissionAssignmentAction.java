...
public class PermissionAssignmentAction extends BaseAction implements ModelDriven<SysRole>
{
	...
	private String queryType = "0";//查询类型
	...
	public String findAllRoleListNotPage() throws Exception{
		...
		try{
			if(Tools.processNull(this.queryType).equals("0")){
				...
				sb.append("select t.role_id CK,t.role_id ROLE_ID,t.name NAME,t.DESCRIPTION DESCRIPTION ");
				sb.append("from sys_role t WHERE t.status = 'A'  ");
				if(!Tools.processNull(rowName).equals("")){
					sb.append("and t.name = '" + rowName + "' ");
				}
				Page list = baseService.pagingQuery(sb.toString(),1,1000000);
	...
	*/
	...
	{	
		...
		map.put("addList", JSON.parseArray(inserted, SysRole.class));
		map.put("updList", JSON.parseArray(updated, SysRole.class));
		map.put("delList", JSON.parseArray(deleted, SysRole.class));
		Json json=new Json();
