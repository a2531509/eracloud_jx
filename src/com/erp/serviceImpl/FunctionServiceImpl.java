...
/**
 * 类功能说明 TODO:程式管理service实现
 * 类修改者
 * 修改日期
 * 修改说明
 * <p>Title: FunctionServiceImpl.java</p>
...
public class FunctionServiceImpl  implements FunctionService
{
	...
	private PublicDao<SysPermission> permissionPublicDao;
	@Autowired
	public void setPermissionPublicDao(PublicDao<SysPermission> permissionPublicDao )
	{
	...
	*/
	public boolean persistenceFunction(List<SysPermission> list ) 
	{
		logger.debug("f");
	...
	*/
	...
	{
		String hql="from SysPermission t where t.status='A' ";
		...
		hql+=" order by t.sort ";
		List<SysPermission> list = permissionPublicDao.find(hql);
	...
	{
		String hql="from SysPermission t where t.status='A' and t.type='F' order by t.sort ";
		List<SysPermission> list = permissionPublicDao.find(hql);
	...
	*/
	...
	{
		String hql=" from SysPermission t where t.status='A' and t.pid="+id;
		List<SysPermission> list = permissionPublicDao.find(hql);
		if (list.size()!=0)
		{
			return false;
		}else {
			SysPermission function = permissionPublicDao.get(SysPermission.class, id);
